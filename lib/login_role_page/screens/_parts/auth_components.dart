...
class AuthField extends StatelessWidget {
  const AuthField({super.key, required this.label, required this.hint, this.obscure = false, this.controller});
  final String label;
  final String hint;
  final bool obscure;
  // controller for Supabase integration - optional, backward compatible
...
class PrimaryAuthButton extends StatelessWidget {
  const PrimaryAuthButton({super.key, required this.label, required this.onPressed, this.isLoading = false});
  final String label;
  final VoidCallback onPressed;
  // isLoading for async Supabase auth calls - optional, backward compatible
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.5)) : Text(label),
      ),
    );
  }
}

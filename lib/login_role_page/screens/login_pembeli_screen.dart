...
import 'daftar_pembeli_screen.dart';
import 'package:menusisa_dev/core/auth/auth_service.dart';
import 'package:menusisa_dev/main_customer_page.dart';

class LoginPembeliScreen extends StatelessWidget {
  const LoginPembeliScreen({super.key});
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Container(
                ...
                child: isDesktop
                    ? Row(children: [const Expanded(child: AuthSidebarArt(asset: 'assets/images/login_buyer.png', title: 'Masuk sebagai\nPembeli')), const Expanded(child: _LoginForm())])
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text('Selamat datang kembali!', style: TextStyle(color: AppColors.textGrey)),
        ...
        const Center(child: Text('atau masuk dengan', style: TextStyle(color: AppColors.textGrey))),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.g_mobiledata), label: const Text('Google'))), const SizedBox(width: 12), Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.facebook), label: const Text('Facebook')))]),
        const SizedBox(height: 28),
        Center(child: TextButton(onPressed: () => Navigator.of(context).push(PageFadeRoute(builder: (_) => const DaftarPembeliScreen())), child: const Text('Belum punya akun? Daftar'))),
      ]),
    );
  }
}
